   ...
   async function explorarMazmorra(usuario) {
      ...
      if (probabilidadMonstruo <= 15) { // Probabilidad de 30% de encontrar un monstruo
         ...
         if (fuerzaUsuario > fuerzaMonstruo) {
            ...
            conn.reply(m.chat, `*[ 😵  ¡Encontraste un monstruo! El monstruo te derrotó y perdiste ➔ ${estrellasEncontradas} Estrellas 💫. ]*\n\n> ${textbot}`, m, rcanal);
         }
      } else {
         global.db.data.users[m.sender].estrellas += estrellasEncontradas; 
         conn.reply(m.chat, `*[ 🥳 ¡Exploraste la casa de Rembo y encontraste ${estrellasEncontradas} Estrellas 💫]*\n\n> ${textbot}`, m, rcanal);
      }
   }
...
handler.tags = ['rpg']
handler.command = ['explorar', 'explo']
